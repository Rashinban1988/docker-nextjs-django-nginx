  ...
  useEffect(() => {
    const fetchUploadedFiles = async () => {
      try {
        ...
        const response = await instance.get('/api/spokenMaterial/uploaded-files/');
      ...
      try {
        await axios.post('http://localhost:8080/api/spokenMaterial/uploaded-files/', formData, {
          ...
          onUploadProgress: (progressEvent) => {
            const percentCompleted = Math.round((progressEvent.loaded * 100) / (progressEvent.total ?? 1));            setUploadProgress(percentCompleted); // アップロード進行状況を更新
          },
        });
        alert('ファイルが正常にアップロードされました。');
        // アップロード後にファイルリストを更新
        const response = await axios.get('/api/spokenMaterial/uploaded-files/');
        setUploadedFiles(response.data);
      ...
      <div className="my-8">
        ...
        <div className="max-w-lg mx-auto my-8 grid grid-cols-1 gap-4">
          {uploadedFiles.map((file) => (
            <div key={file.id} className="bg-white rounded-lg shadow group hover:shadow-md hover:bg-yellow-200 overflow-hidden cursor-pointer group" onClick={() => window.location.href=`/transcription/${file.id}`}>
